...
class ProtocolRun:
	...
	def move_volume(self,reagent, source, dest, vol, air_gap_vol, 
		...
		self.ctx.delay(seconds = reagent.delay) # pause for x seconds depending on reagent
		if blow_out == True:
	...
	def calc_height(self,reagent, cross_section_area, aspirate_volume, 
		...
		if reagent.vol_well < aspirate_volume + extra_volume:
			...
			if (height < min_height) :
			 	height = min_height
			col_change = True
			self.comment('Remaining volume now will be:' + str(reagent.vol_well))
...
def run(ctx: protocol_api.ProtocolContext):
	...
	if (run.next_step()):
		...
		for dest in aw_wells:
			[pickup_height, col_change] = run.calc_height(liquid,4.12*4.12*math.pi,volumen_move)
	...
	if (run.next_step()):
		...
		for destination in aw_wells_multi:
			...
			vol = 150
			vol_min = 1000
			[pickup_height,col_change]=run.calc_height(beads, pool_area, vol*8,extra_volume = vol_min)
