...
def run(protocol: protocol_api.ProtocolContext):
    ...
    tiprack1 = protocol.load_labware('opentrons_96_tiprack_200ul', 3)
    ...
    plate_temp = temp_mod.load_labware(
        'opentrons_24_aluminumblock_generic_2ml_screwcap')

    # # mezclando los reactivos del modulo de temperatura
    right.transfer(r1, plate_temp.wells('A1'), plate_temp.wells('D1'))
    right.transfer(r2, plate_temp.wells('A2'),
                   plate_temp.wells('D1'), mix_before=(2, 20))
    right.transfer(r3, plate_temp.wells('A3'),
                   plate_temp.wells('D1'), mix_before=(2, 20))
